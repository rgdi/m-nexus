    ...
    async (req) => {
      ...
      return {
        ...
        stats: {
          ...
          byMastery: {
            low: allConcepts(g).filter((c) => Object.values(c.layers).some((l) => l.mastery < 0.4)).length,
            mid: allConcepts(g).filter((c) => Object.values(c.layers).some((l) => l.mastery >= 0.4 && l.mastery < 0.8)).length,
            high: allConcepts(g).filter((c) => Object.values(c.layers).every((l) => l.mastery >= 0.8)).length,
          },
        },
      };
    },
