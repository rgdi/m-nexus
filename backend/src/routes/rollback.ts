...
export async function rollbackRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: { version?: string; description?: string; trigger?: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // Usamos tar en lugar de zip para mejor compresión y metadata
        ...
        if (!existsSync(backupPath)) {
          return reply.code(500).send({ code: "BACKUP_FAILED", message: "tar command produced no output" });
        }
        ...
        writeRegistry(items);
        logger.info({ id, size: stat.size, trigger: info.trigger }, "Backup created");
        return reply.send({ backup: info, total: items.length });
      } catch (err) {
        logger.error({ err }, "Backup creation failed");
        return reply.code(500).send({ code: "BACKUP_FAILED", message: (err as Error).message });
      }
    }
  );
  ...
  app.post<{ Body: { id: string; confirm?: boolean } }>(
    ...
    async (req, reply) => {
      ...
      if (!id) return reply.code(400).send({ code: "INVALID", message: "id required" });
      if (!req.body.confirm) {
        return reply.code(400).send({ code: "CONFIRM_REQUIRED", message: "Set confirm:true to proceed" });
      }
      const items = readRegistry();
      const item = items.find((b) => b.id === id);
      if (!item) return reply.code(404).send({ code: "NOT_FOUND" });
      if (!existsSync(item.path)) {
        return reply.code(410).send({ code: "BACKUP_FILE_GONE", message: "Backup file no longer exists" });
      }
      try {
        // Restaurar: extrae el tar.gz en dataDir()
        // OJO: esto SOBREESCRIBE los datos actuales. El caller debe haber
        // confirmado. Hacemos un backup "pre-restore" por seguridad.
        ...
        logger.warn({ id, restoreBackupPath }, "Rollback applied");
        return reply.send({
          ok: true,
          restored: item,
        ...
        logger.error({ err, id }, "Rollback failed");
        return reply.code(500).send({ code: "ROLLBACK_FAILED", message: (err as Error).message });
      }
    }
  );
